...
[[ -z "$NAME" ]] && export NAME="${{TOKEN_NAME}}"
[[ -z "$SYMBOL" ]] && export SYMBOL="${{TOKEN_SYMBOL}}"
#
...
#
[[ -z "$LETTER" ]] && export LETTER="${{ILK_LETTER}}"
...
# estimate
[ "$ESTIMATE" = "true" ] && {
	RESPONSE=$($FORGE_SCRIPT "${BASH_SOURCE%/*}/${{TOKEN_SYMBOL}}Deployment.s.sol:${{TOKEN_SYMBOL}}Deployment" | tee >(cat 1>&2))
	jq -R 'fromjson? | .logs | .[] | fromjson?' <<<"$RESPONSE" | jq -s 'map( {(.[0]): .[1]} ) | add'
	exit 0
}

RESPONSE=$($FORGE_SCRIPT "${BASH_SOURCE%/*}/${{TOKEN_SYMBOL}}Deployment.s.sol:${{TOKEN_SYMBOL}}Deployment" --broadcast --slow --verify --retries 10 | tee >(cat 1>&2))
jq -R 'fromjson? | .logs | .[] | fromjson?' <<<"$RESPONSE" | jq -s 'map( {(.[0]): .[1]} ) | add'
